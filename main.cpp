...
Chunk chunk_from_strings(const std::string& chunk_type_str, const std::string& data) {
    ChunkType chunk_type = ChunkType::fromStr(chunk_type_str);
    std::vector<uint8_t> bytes(data.begin(), data.end());
    return Chunk(chunk_type, bytes);
}

void test_from_chunks() {
    std::vector<Chunk> chunks;
    
    // Create first chunk
    std::string message1 = "I am the first chunk";
    std::vector<uint8_t> data1(message1.begin(), message1.end());
    chunks.push_back(Chunk(ChunkType::fromStr("FrSt"), data1));
    
    // Create second chunk
    std::string message2 = "I am another chunk";
    std::vector<uint8_t> data2(message2.begin(), message2.end());
    chunks.push_back(Chunk(ChunkType::fromStr("miDl"), data2));
    
    // Create third chunk
    std::string message3 = "I am the last chunk";
    std::vector<uint8_t> data3(message3.begin(), message3.end());
    chunks.push_back(Chunk(ChunkType::fromStr("LASt"), data3));
    
    PNG png(chunks);
    assert(png.chunks().size() == 3);
}

void test_valid_from_bytes() {
    std::vector<Chunk> chunks;
    std::string message = "I am the first chunk";
    std::vector<uint8_t> data(message.begin(), message.end());
    
    Chunk initial_chunk(ChunkType::fromStr("FrSt"), data);
    chunks.push_back(initial_chunk);
    
    auto chunk_bytes = initial_chunk.as_bytes();
    
    std::vector<uint8_t> bytes;
    bytes.insert(bytes.end(), PNG::STANDARD_HEADER.begin(), PNG::STANDARD_HEADER.end());
...
void test_invalid_header() {
    ...
    std::string message = "I am the first chunk";
    std::vector<uint8_t> data(message.begin(), message.end());
    chunks.push_back(Chunk(ChunkType::fromStr("FrSt"), data));
    
    std::vector<uint8_t> chunk_bytes;
    for (const auto& chunk : chunks) {
        auto chunk_data = chunk.as_bytes();
        chunk_bytes.insert(chunk_bytes.end(), chunk_data.begin(), chunk_data.end());
    }

    std::vector<uint8_t> invalid_header = {13, 80, 78, 71, 13, 10, 26, 10};
    std::vector<uint8_t> bytes;
    bytes.insert(bytes.end(), invalid_header.begin(), invalid_header.end());
    bytes.insert(bytes.end(), chunk_bytes.begin(), chunk_bytes.end());
...
void test_invalid_chunk() {
    // Bad chunk data
    std::vector<uint8_t> bad_chunk = {
        0, 0, 0, 5,         // length
...
void test_list_chunks() {
    ...
    PNG png(chunks);
    assert(png.chunks().size() == 1);
}

void test_chunk_by_type() {
    std::vector<Chunk> chunks;
    std::string message = "I am the first chunk";
    std::vector<uint8_t> data(message.begin(), message.end());
    chunks.push_back(Chunk(ChunkType::fromStr("FrSt"), data));
    ...
    PNG png(chunks);
    auto chunk_opt = png.chunk_by_type(ChunkType::fromStr("FrSt"));
    assert(chunk_opt.has_value());
    Chunk chunk = chunk_opt.value();
    assert(chunk.chunktype().toString() == "FrSt");
    assert(chunk.data_as_string() == "I am the first chunk");
}

void test_append_chunk() {
    std::vector<Chunk> chunks;
    std::string init_message = "Initial chunk";
    std::vector<uint8_t> init_data(init_message.begin(), init_message.end());
    chunks.push_back(Chunk(ChunkType::fromStr("FrSt"), init_data));
    
    PNG png(chunks);
    
    std::string message = "Message";
    std::vector<uint8_t> data(message.begin(), message.end());
    Chunk new_chunk(ChunkType::fromStr("TeSt"), data);
    
    png.append_chunk(new_chunk);
    auto chunk_opt = png.chunk_by_type(ChunkType::fromStr("TeSt"));
    assert(chunk_opt.has_value());
    Chunk chunk = chunk_opt.value();
    assert(chunk.chunktype().toString() == "TeSt");
    assert(chunk.data_as_string() == "Message");
}

void test_remove_first_chunk() {
    std::vector<Chunk> chunks;
    std::string init_message = "Initial chunk";
    std::vector<uint8_t> init_data(init_message.begin(), init_message.end());
    chunks.push_back(Chunk(ChunkType::fromStr("FrSt"), init_data));
    ...
    std::string message = "Message";
    std::vector<uint8_t> data(message.begin(), message.end());
    chunks.push_back(Chunk(ChunkType::fromStr("TeSt"), data));
    
    PNG png(chunks);
    png.remove_first_chunk(ChunkType::fromStr("TeSt"));
    auto chunk_opt = png.chunk_by_type(ChunkType::fromStr("TeSt"));
    assert(!chunk_opt.has_value());
    
    auto init_chunk_opt = png.chunk_by_type(ChunkType::fromStr("FrSt"));
    assert(init_chunk_opt.has_value());
}

void test_png_from_image_file() {
    std::vector<uint8_t> png_data(PNG_FILE, PNG_FILE + sizeof(PNG_FILE));
    PNG png(png_data);
    assert(true);
}
...
void test_as_bytes() {
    std::vector<uint8_t> png_data(PNG_FILE, PNG_FILE + sizeof(PNG_FILE));
    PNG png(png_data);
    auto bytes = png.as_bytes();
    assert(bytes == png_data);
}

void test_png_trait_impls() {
    std::vector<Chunk> chunks;
